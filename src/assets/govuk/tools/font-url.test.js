...
describe('@function font-url', () => {
  it('by default concatenates the font path and the filename', async () => {
    ...
    expect(results.css.toString().trim()).toEqual('@font-face{font-family:"whatever";src:url("/path/to/fonts/whatever.woff2")}');
  });
  ...
  it('can be overridden to use a defined Sass function', async () => {
    ...
    expect(results.css.toString().trim()).toEqual('@font-face{font-family:"whatever";src:"WHATEVER.WOFF2"}');
  });
  ...
  it('can be overridden to use a custom function', async () => {
    ...
    expect(results.css.toString().trim()).toEqual('@font-face{font-family:"whatever";src:url("/custom/whatever.woff2")}');
  });
});
