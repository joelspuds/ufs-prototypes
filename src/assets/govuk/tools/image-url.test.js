...
describe('@function image-url', () => {
  it('by default concatenates the image path and the filename', async () => {
    ...
    expect(results.css.toString().trim()).toEqual('.foo{background-image:url("/path/to/images/baz.png")}');
  });
  ...
  it('can be overridden to use a defined Sass function', async () => {
    ...
    expect(results.css.toString().trim()).toEqual('.foo{background-image:"BAZ.PNG"}');
  });
  ...
  it('can be overridden to use a custom function', async () => {
    ...
    expect(results.css.toString().trim()).toEqual('.foo{background-image:url("/custom/baz.png")}');
  });
});
