...
describe('Table', () => {
  ...
  describe('column headers', () => {
    it('can be specified', () => {
      ...
      expect(headings).toEqual(['Month you apply', 'Rate for bicycles', 'Rate for vehicles']);
    });
  ...
  describe('row headers', () => {
    describe('when firstCellIsHeader is false', () => {
      it('are not included', () => {
        const $ = render('table', {
          rows: [
            [{ text: 'Apples' }, { text: 'green' }],
            [{ text: 'Strawberries' }, { text: 'red' }],
        ...
        expect(cells).toEqual(['Apples', 'green', 'Strawberries', 'red', 'Bananas', 'yellow']);
      });
    });
    ...
    describe('when firstCellIsHeader is true', () => {
      ...
      it('have HTML escaped when passed as text', () => {
        const $ = render('table', {
          firstCellIsHeader: true,
          rows: [[{ text: 'Foo <script>hacking.do(1337)</script>' }]],
        });
      ...
      it('can have additional classes', () => {
        const $ = render('table', {
          firstCellIsHeader: true,
          rows: [[{ text: 'Foo', classes: 'my-custom-class' }, { text: 'bar' }]],
        });
      ...
      it('can have rowspan specified', () => {
        const $ = render('table', {
          firstCellIsHeader: true,
          rows: [[{ text: 'Foo', rowspan: 2 }, { text: 'bar' }]],
        });
      ...
      it('can have colspan specified', () => {
        const $ = render('table', {
          firstCellIsHeader: true,
          rows: [[{ text: 'Foo', colspan: 2 }, { text: 'bar' }]],
        });
      ...
      it('can have additional attributes', () => {
        const $ = render('table', {
          firstCellIsHeader: true,
          rows: [[{ text: 'Foo', attributes: { 'data-fizz': 'buzz' } }, { text: 'bar' }]],
        });
  ...
  describe('cells', () => {
    it('can be specified', () => {
      const $ = render('table', {
        rows: [[{ text: 'A' }, { text: '1' }], [{ text: 'B' }, { text: '2' }], [{ text: 'C' }, { text: '3' }]],
      });
      ...
      expect(cells).toEqual([['A', '1'], ['B', '2'], ['C', '3']]);
    });

    it('can be skipped when falsely', () => {
      const $ = render('table', {
        rows: [[{ text: 'A' }, { text: '1' }], null, undefined, false, [{ text: 'B' }, { text: '2' }], [{ text: 'C' }, { text: '3' }]],
      });
